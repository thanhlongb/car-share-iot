...
def get_line_chart_data():
    ...
    data = [r.serialize_with_cols(['id', 'role', 'date']) for r in users]
    ...
    for i in range(len(indexes)):
        if indexes[i] < min_date:
            users_group_by_date.drop(labels=[indexes[i]], inplace = True)
        index_split = indexes[i].split('-')
        new_index = index_split[1] + '/' + index_split[2]
        users_group_by_date.rename({indexes[i]:new_index}, inplace=True)
...
def get_bar_chart_data():
    cars = Car.query.all()
    cars_list = [r.serialize_with_cols(['id', 'make']) for r in cars]
    bookings = Booking.query.all()
    bookings_list = [r.serialize_with_cols(['id', 'car_id']) for r in bookings]
    ten_days_ago = datetime.now() - timedelta(days=10)
    booking_action = BookingAction.query.filter(BookingAction.creation_time >= ten_days_ago).all()
    booking_acion_list = [r.serialize_with_cols(['id', 'booking_id', 'action']) for r in booking_action]
    cars_df = pd.DataFrame(cars_list)
    bookings_df = pd.DataFrame(bookings_list)
    booking_action_df = pd.DataFrame(booking_acion_list)
    bookings_df.rename(columns={"id":"booking_id"}, inplace=True)
    cars_df.rename(columns={"id":"car_id"}, inplace=True)
    booking_created = booking_action_df[booking_action_df.action=='created']
    booking_created.drop(columns=['action'], inplace=True)
    booking_merged = booking_created.merge(bookings_df, on="booking_id")
    booking_car_merged = booking_merged.merge(cars_df, on="car_id")
